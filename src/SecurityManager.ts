...
async function getSecretStorageKey(
   ...
   let miks_embedded = SettingsStore.getValue("feature_miks_embedded")
   if (!miks_embedded) {
      ...
      const getInput = async () => {
         return await new Promise((resovle, reject) => {
            return (async () => {
               let _input = await new Promise((_resolve, _reject) => {
                  UIStore.instance.emit('REQUEST_MIKS_MASTERKEY', _resolve, _reject);
               }), key = await inputToKey(_input),
                  matches = await MatrixClientPeg.get().checkSecretStorageKey(key, keyInfo);
               if (matches) {
                  return resovle(_input);
               } else {
                  await new Promise((_resolve, _reject) => {
                     UIStore.instance.emit('MIKS_MASTERKEY_ERROR', _resolve, _reject);
                  })
                  return resovle(await getInput());
               }
            })()
         })
      }
      let input = await getInput();
      let key = await inputToKey(input);
      localStorage.setItem('mx_miks_masterkey', input.passphrase);
      UIStore.instance.emit('MIKS_MASTERKEY_SUCCESS')
